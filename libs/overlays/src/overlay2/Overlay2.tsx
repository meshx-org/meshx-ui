...
const Classes = {
    ...
    OVERLAY_INLINE: `overlay-inline`,
    OVERLAY_BACKDROP: `overlay-backdrop`,
    OVERLAY_CONTENT: `overlay-content`,
    OVERLAY_END_FOCUS_TRAP: `overlay-end-focus-trap`,
    OVERLAY_START_FOCUS_TRAP: `overlay-start-focus-trap`
}

/** Overlay component. */
export class Overlay2 extends AbstractPureComponent<OverlayProps2, OverlayState> {
    public static displayName = `Overlay2`

    public static defaultProps: OverlayProps2 = {
        autoFocus: true,
        backdropProps: {},
    ...
    public static getDerivedStateFromProps({ isOpen: hasEverOpened }: OverlayProps2) {
        if (hasEverOpened) {
    ...
    private static openStack: Overlay2[] = []
    private static getLastOpened = () => Overlay2.openStack[Overlay2.openStack.length - 1]
    ...
    public componentDidUpdate(prevProps: OverlayProps2) {
    ...
    private overlayWillClose() {
        ...
        const { openStack } = Overlay2
        const stackIndex = openStack.indexOf(this)
        if (stackIndex !== -1) {
            openStack.splice(stackIndex, 1)
            if (openStack.length > 0) {
                const lastOpenedOverlay = Overlay2.getLastOpened()
    ...
    private overlayWillOpen() {
        const { getLastOpened, openStack } = Overlay2
        if (openStack.length > 0) {
    ...
    private handleDocumentClick = (e: MouseEvent) => {
        ...
        const stackIndex = Overlay2.openStack.indexOf(this)
        const isClickInThisOverlayOrDescendant = Overlay2.openStack
            .slice(stackIndex)
