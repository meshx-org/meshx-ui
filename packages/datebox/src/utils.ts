...
import { DatePickerProps, DateValue, Granularity, TimeValue } from './DateBox.types'
import { useRef } from 'react'

export function isInvalid(value: DateValue, minValue: DateValue, maxValue: DateValue) {
    return (
        value != null &&
        ((minValue != null && value.compare(minValue) < 0) || (maxValue != null && value.compare(maxValue) > 0))
    )
}
...
export function getFormatOptions(fieldOptions: FieldOptions, options: FormatterOptions): Intl.DateTimeFormatOptions {
    ...
    let opts: Intl.DateTimeFormatOptions = keys.slice(startIdx, endIdx + 1).reduce<any>((opts, key) => {
        opts[key] = fieldOptions[key as keyof FieldOptions]  
        return opts
    }, {})
