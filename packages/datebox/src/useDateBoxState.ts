/*
 * Copyright 2023 MeshX. All rights reserved.
...
import { DatePickerProps, DateValue, Granularity } from './DateBox.types'
...
const EDITABLE_SEGMENTS: Record<keyof Intl.DateTimeFormatPartTypesRegistry, boolean> = {
    year: true,
    month: true,
    day: true,
    hour: true,
...
const PAGE_STEP: Record<keyof Intl.DateTimeFormatPartTypesRegistry, number | undefined> = {
    year: 5,
    month: 2,
    day: 7,
    hour: 2,
...
const TYPE_MAPPING: Record<string, keyof Intl.DateTimeFormatPartTypesRegistry> = {
    dayperiod: 'dayPeriod'
}
...
export function useDateFieldState(props: DateFieldStateOptions): DateFieldState {
    const { locale, createCalendar, hideTimeZone, isDisabled, isReadOnly, isRequired } = props

    const v: DateValue = props.value || props.defaultValue || props.placeholderValue
    ...
    const [value, setDate] = useControlledState<DateValue>(props.value, props.defaultValue, props.onChange)

    const calendarValue = useMemo(() => convertValue(value, calendar), [value, calendar])

    // We keep track of the placeholder date separately in state so that onChange is not called
    // until all segments are set. If the value === null (not undefined), then assume the component
    // is controlled, so use the placeholder as the value until all segments are entered so it doesn't
    // change from uncontrolled to controlled and emit a warning.
    const [placeholderDate, setPlaceholderDate] = useState<AnyCalendarDate>(() =>
    ...
    const opts = useMemo(() => getFormatOptions({}, formatOpts), [formatOpts])

    const dateFormatter = useMemo(() => new DateFormatter(locale, opts), [locale, opts])
    const resolvedOptions = useMemo(() => dateFormatter.resolvedOptions(), [dateFormatter])
    ...
    const allSegments = useMemo(
        () =>
            dateFormatter
                ...
                .reduce<Partial<typeof EDITABLE_SEGMENTS>>((p, seg) => ((p[seg.type] = true), p), {}),
        [dateFormatter]
    )
    ...
    let displayValue =
        calendarValue && Object.keys(validSegments).length >= Object.keys(allSegments).length
            ? calendarValue
            : placeholderDate
    ...
    let validationState: ValidationState =
        props.validationState || (isInvalid(calendarValue, props.minValue, props.maxValue) ? 'invalid' : null)
    ...
    return {
        ...
        setSegment(part, v) {
            markValid(part)
            setValue(setSegment(displayValue, part, v as number, resolvedOptions))
        },
        confirmPlaceholder() {
            if (props.isDisabled || props.isReadOnly) {
                return
        ...
        clearSegment(part) {
            ...
            const placeholder = createPlaceholderDate(props.placeholderValue, granularity, calendar, defaultTimeZone)
        ...
        formatValue(fieldOptions: FieldOptions) {
            ...
            const formatter = new DateFormatter(locale, getFormatOptions(fieldOptions, formatOpts))
